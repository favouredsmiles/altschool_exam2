...
echo -e "updating linux server\n"
sudo apt update -y

#install your apache webserver
echo -e "installing apache2 on linux server\n"
...
echo -e "installing composer on VM\n"
cd /usr/bin
...
#change directory in /var/www directory and clone the laravel repo 
...
sudo mysql -uroot -e "CREATE DATABASE Vincent;"
sudo mysql -uroot -e "CREATE USER 'Favour'@'localhost' IDENTIFIED BY 'ewere';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Vincent.* TO 'Favour'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Vincent/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=Favour/' /var/www/laravel/.env
sudo sed -i '27 s/=/=ewere/' /var/www/laravel/.env
